...
const Locations = () => {
  ...
  // Remove direct DOM manipulation, use React state for hover overlays
  ...
  return (
    <div className="available-locations">
      {/* Overlay buttons for venues, shown on hover */}
      {venuePolygons.map((venue, idx) => (
        <div
          key={venue.id}
          className={`${venue.id}-button-overlay`}
          style={{ opacity: hoveredVenue === venue.id ? 1 : 0 }}
        >
          <button>{locations[idx]?.name || `Venue ${idx + 1}`}</button>
        </div>
      ))}
      ...
      >
        ...
        ></image>
        {venuePolygons.map((venue, idx) => (
          <a href={venue.href} key={venue.id}>
            <polygon
              id={venue.id}
              points={venue.points}
              onMouseOver={() => setHoveredVenue(venue.id)}
